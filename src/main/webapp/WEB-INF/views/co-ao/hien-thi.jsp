        ...
        <div class="card-body">
            <div>
                <ul class="nav nav-tabs border-top" id="setting-panel" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="/co-ao/view-add" role="tab" onclick="if(!(confirm('Bạn có muốn thực hiện thao tác này không ? ')))return false;">Thêm
                            thông tin
                            cổ áo</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="/co-ao/hien-thi-delete" role="tab" onclick="if(!(confirm('Bạn có muốn thực hiện thao tác này không ? ')))return false;">
            ...
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade show active" id="description" role="tabpanel"
                    ...
                    <div class="col-lg-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="float: left">Danh sách cổ áo</h4>
                                <%--            Tìm kiếm               --%>
                                <form action="/co-ao/search-con-hoat-dong" method="post">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped" style="color: black">
                                        ...
                                        <br>
                                        <br>
                                        <c:forEach items="${listCoAo}" var="coAo" varStatus="index">
                                            <tr>
                                                <td>${index.index+1}</td>
                                                <td>${coAo.ma}</td>
                                                <td>${coAo.ten}</td>
                                                <td>
                                                    <c:if test="${coAo.trangThai==0}">Còn sử dụng</c:if>
                                                    <c:if test="${coAo.trangThai==1}">Ngừng sử dụng</c:if>
                                                </td>
                                                <td>${coAo.ngayTao}</td>
                                                <td>
                                                    <a href="/co-ao/detail/${coAo.id}"
                                                       ...
                                                       onclick="if(!(confirm('Bạn có muốn thực hiện thao tác này không ? ')))return false;">
                                                        ...
                                                        Detail</a>
                                                    <a href="/co-ao/view-update/${coAo.id}"
                                                       class="btn btn-warning btn-icon-text"
                                                       tabindex="-1"
                                                       role="button"
                                                       onclick="if(!(confirm('Bạn có muốn thực hiện thao tác này không ? ')))return false;">
                                    ...
                                    <c:if test="${not listCoAo.isEmpty()}">
                                        <nav aria-label="Page navigation example">
                                            <ul class="pagination justify-content-center pagination-lg">
                                                <c:forEach begin="1" end="${totalPage}" varStatus="status">
                                                    <li class="page-item">
                                                        <a href="/co-ao/hien-thi?pageNum=${status.index -1}"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body" style="color: black">
                <form:form action="/co-ao/add" method="post" modelAttribute="coAo">
...
<button style="display: none" type="button" id="batmodaldetailcoao" class="btn btn-primary" data-bs-toggle="modal"
...
<div class="modal" id="detailCoAo">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body" style="color: black">
                <form:form method="post" modelAttribute="coAo">
                    <div class="form-group">
                        <form:label class="form-label" path="ma">Mã cổ áo:</form:label>
...
<button style="display: none" type="button" id="batmodalupdatecoao" class="btn btn-primary" data-bs-toggle="modal"
...
<div class="modal" id="updateCoAo">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body" style="color: black">
                <form:form action="/co-ao/update/${coAo.id}" method="post" modelAttribute="coAo">
                    <div class="form-group">
                        <form:label class="form-label" path="ma">Mã cổ áo:</form:label>
                        <form:input class="form-control" path="ma" readonly="true"/>
                        <form:errors path="ma" cssStyle="color: red"></form:errors>
...
<script>
    <c:if test="${batmodalthemcoao==0}">
    document.getElementById('batmodalthemcoao').click();
    </c:if>
</script>
<script>
    <c:if test="${batmodaldetailcoao==0}">
    document.getElementById('batmodaldetailcoao').click();
    </c:if>
</script>
<script>
    <c:if test="${batmodalupdatecoao==0}">
    document.getElementById('batmodalupdatecoao').click();
    </c:if>
</script>
</html>
